...
const ScoreModal = ({ onClose, onAddScore, scoresList, editIndex }: ScoreModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50">
      <div className="bg-gray-700 p-4 rounded-lg mb-40 shadow-lg shadow-black">
        <h2 className="text-lg font-semibold mb-2">Ingresar Nuevo puntaje</h2>
        <div className="flex justify-between mb-4 ">
          <div className="flex flex-col justify-between items-center mb-2">
            <label className="mr-2 text-white">Nos:</label>
            <input
              type="number"
              placeholder="0"
              min={0}
              onChange={(e) => handleChange(e, "Nos")}
              className="border rounded-lg p-2 w-16 text-center text-white bg-gray-700"
            />
          </div>
          <div className="flex flex-col justify-between items-center mb-2">
            <label className="mr-2 text-white">Ellos:</label>
            <input
              type="number"
              min={0}
              placeholder="0"
              onChange={(e) => handleChange(e, "Ellos")}
              className="border show rounded-lg p-2 w-16 text-center text-white bg-gray-700"
            />
          </div>
        </div>
        <button onClick={handleSubmit} className=" bg-blue-600 bg-opacity-35 text-white p-2 rounded-lg w-full">
          {editIndex !== null ? "Editar" : "Guardar"}
        </button>
        <button onClick={onClose} className="mt-2 bg-red-600 bg-opacity-35 text-white p-2 rounded-lg w-full">
          Cancelar
        </button>
      </div>
    </div>
